...
// Particle cloud weights
#include "amcl/ParticleCloudWeights.h"
...
class AmclNode
{
  ...
  private:
    ...
    ros::Publisher particlecloud_weights_pub_;
        ...
        first_reconfigure_call_(true)
{
  ...
  particlecloud_weights_pub_ = nh_.advertise<amcl::ParticleCloudWeights>("particlecloud_weights", 2, true);
...
AmclNode::laserReceived(const sensor_msgs::LaserScanConstPtr& laser_scan)
{
  ...
  if(lasers_update_[laser_index])
  {
    ...
    if (!m_force_update) {
      ...
      // Build a message with the particle cloud weights and publish
      // it out.  Make sure it has the same time stamp as the cloud so
      // that we can corellate them
      amcl::ParticleCloudWeights cloud_weights_msg;
      cloud_weights_msg.header.stamp = cloud_msg.header.stamp;
      cloud_weights_msg.weights.resize(set->sample_count);
      for(int i = 0; i < set->sample_count; ++i) {
	cloud_weights_msg.weights[i].data = set->samples[i].weight;
      }
      particlecloud_weights_pub_.publish(cloud_weights_msg);
    }
  }
