// Replaces foreground stuff with black pixels; it's a redaction filter.
// Chooses which foreground thing to redact via green hat. 
...
void cloud_callback (pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud_fore)
{
  if (is_first_cloud)
      ...
      for (int i = 0; i != points_clusters->size(); ++i)
	{
      ...
      // Remove biggest cluster from foreground image by turning it BLACK
      redact_indices(clusters[closest].indices, cloud_show);
