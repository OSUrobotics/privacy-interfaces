...
class Projector():
    ...
    def bounds_callback(self, bounds):
        if self.need_bounds:
            ...
            for i in range(len(bounds.polygon.points)):
                point_stamped = Vector3Stamped()
                point_stamped.header.frame_id = bounds.header.frame_id
                point_stamped.header.stamp = rospy.Time(0)
                [point_stamped.vector.x,
                 point_stamped.vector.y,
                 point_stamped.vector.z] = [bounds.polygon.points[i].x,
                                           bounds.polygon.points[i].y,
                                           bounds.polygon.points[i].z]
                self.lis.waitForTransform(bounds.header.frame_id,
                                          '/map',
                                          rospy.Time(0),
                                          rospy.Duration(3.0))
                point_stamped = self.lis.transformVector3('/map', point_stamped)
                [bounds.polygon.points[i].x,
                 bounds.polygon.points[i].y,
                 bounds.polygon.points[i].z] = [point_stamped.vector.x,
                                                point_stamped.vector.y,
                                                point_stamped.vector.z]
    ...
    def image_callback(self, image):
        ...
        if not any([self.need_bounds, self.need_info, self.need_amcl]):
            ...
            for pose in self.amcl.poses:
                ...
                # pose.orientation.y,
                # pose.orientation.z,
                # pose.orientation.w] = quaternion_multiply(q_pose,
                #                                           rotation)

                # Convert to PoseStamped for logistical reasons
                camera = PoseStamped()
